                        ...
                        <?php foreach($children as $child): ?>
                            <div style="background: var(--primary-lightest); padding: 1rem; border-radius: 12px; display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" style="display: inline;" data-no-ajax>
                                    ...
                                    <button type="submit" class="btn btn-danger btn-sm" onclick="event.preventDefault(); var f=this.closest('form'); showConfirmModal('Remove Child', 'Are you sure you want to remove this child record?', {type: 'destructive', confirmText: 'Remove'}, function(ok){ if(ok) f.submit(); });">
